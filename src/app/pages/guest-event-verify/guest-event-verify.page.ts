...
@Component({
  selector: 'app-guest-event-verify',
  templateUrl: './guest-event-verify.page.html',
  styleUrls: ['./guest-event-verify.page.scss'],
})
export class GuestEventVerifyPage implements OnInit {
  user: any;
  unverifiedUsers: any[]=[];
  ...
  async verifyUser(uid: string) {
    ...
    const unverifiedUsersRef = this.firestore.collection('users', ref => ref.where('uid', '==', uid).where('verified', '==', false).where('registrationType', '==', 'GuestEvent'));
