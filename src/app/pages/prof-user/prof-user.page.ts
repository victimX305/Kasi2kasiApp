...
@Component({
  ...
  templateUrl: './prof-user.page.html',
  styleUrls: ['./prof-user.page.scss'],
})
export class ProfUserPage implements OnInit {
...
async updateProPicForCurrentUser(proPicUrl: string): Promise<void> {
  ...
  if (user) {
    ...
    try {
      const querySnapshot = await this.firestore.collection('users', ref => ref.where('email', '==', userEmail)).get().toPromise();
