...
@Component({
  selector: 'app-guest-event-prof',
  templateUrl: './guest-event-prof.page.html',
  styleUrls: ['./guest-event-prof.page.scss'],
})
export class GuestEventProfPage implements OnInit {
...
async updateProPicForCurrentUser(proPicUrl: string): Promise<void> {
  ...
  if (user) {
    ...
    try {
      const querySnapshot = await this.firestore.collection('users', ref => ref.where('email', '==', userEmail).where('registrationType', '==', 'GuestEvent')).get().toPromise();
