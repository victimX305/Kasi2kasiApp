...
@Component({
  selector: 'app-prof-guest',
  templateUrl: './prof-guest.page.html',
  styleUrls: ['./prof-guest.page.scss'],
})
export class ProfGuestPage implements OnInit {
      ...
      if (user) {
        ...
        this.firestore.collection('users', ref => ref.where('uid', '==', uid)).valueChanges().subscribe(userData => {
          if (userData.length > 0) {
            ...
            this.firestore.collection('EventOrganizer', ref => ref.where('uid', '==', uid)).valueChanges().subscribe(eventOrganizerData => {
              if (eventOrganizerData.length > 0) {
                this.user = eventOrganizerData[0];
                console.log('User Data (EventOrganizer):', eventOrganizerData[0]);
              } else {
...
async updateProPicForCurrentUser(proPicUrl: string): Promise<void> {
  ...
  if (user) {
    ...
    try {
      const querySnapshot = await this.firestore.collection('users', ref => ref.where('email', '==', userEmail).where('registrationType', '==', 'guest')).get().toPromise();
